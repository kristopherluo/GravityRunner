...
void TileHandler::tile_render(SDL_Renderer* obj_renderer){
    tile_rect[0].x = ((screen_left/2) % tile_rect[0].w) - tile_rect[0].w;
    while(tile_rect[0].x >= (-1 * tile_rect[0].w) && tile_rect[0].x <= tile_rect[0].w){ //while loop that goes through all the ground1 tiles and prints the ones in the camera view
        SDL_RenderCopy(obj_renderer, tile_texture[0], NULL, &tile_rect[0]);
        tile_rect[0].x += tile_rect[0].w;
    }

    //SDL_RenderCopy(obj_renderer, tile_texture[0], NULL, &tile_rect[0]);
    tile_rect[1].x = (screen_left % tile_rect[1].w) - tile_rect[1].w;
    tile_rect[2].x = (screen_left % tile_rect[2].w) - tile_rect[2].w;
    while(tile_rect[1].x >= (-1 * tile_rect[1].w) && tile_rect[1].x <= tile_rect[0].w){ //while loop that goes through all the ground1 tiles and prints the ones in the camera view
