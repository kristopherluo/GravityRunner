...
TileHandler::TileHandler(){
    ...
    tile_rect[2].y = 410; //rect initialization for ground1
    tile_rect[2].w = 40;
    tile_rect[2].h = 40;

    tile_rect[3].y = 70; //rect initialization for background
    tile_rect[3].w = 320; //width of the screen
    tile_rect[3].h = 340; //height of the screen

    tile_rect[4].y = 330; //rect initialization for background
    tile_rect[4].w = 80; //width of the screen
    tile_rect[4].h = 80; //height of the screen
...
void TileHandler::tile_render(SDL_Renderer* obj_renderer){
    ...
    tile_rect[4].y = 170;
    tile_rect[4].x = screen_left % tile_rect[4].w;
    while(tile_rect[4].x <= SCREEN_WIDTH){ //while loop that goes through all the bottom ground tiles and prints the ones in the camera view
        SDL_RenderCopy(obj_renderer, tile_texture[4], NULL, &tile_rect[4]);
        tile_rect[4].x += tile_rect[4].w;
    }*/

    tile_rect[2].y = 410;
    tile_rect[2].x = screen_left % tile_rect[2].w;
    tile_rect[1].x = screen_left % tile_rect[1].w;
    while(tile_rect[2].x <= SCREEN_WIDTH){ //while loop that goes through all the bottom ground tiles and prints the ones in the camera view
        SDL_RenderCopy(obj_renderer, tile_texture[2], NULL, &tile_rect[2]);
        tile_rect[1].y = tile_rect[2].y + tile_rect[2].w;
    ...
    tile_rect[3].x = screen_left/2;
    while(tile_rect[3].x <= SCREEN_WIDTH){ //while loop that goes through all the backdrop tiles and prints the ones in the camera view
        SDL_RenderCopy(obj_renderer, tile_texture[3], NULL, &tile_rect[3]);
        tile_rect[3].x += tile_rect[3].w;
    }

    tile_rect[4].x = screen_left/2 + 120;
    while(tile_rect[4].x <= SCREEN_WIDTH){
        SDL_RenderCopy(obj_renderer, tile_texture[4], NULL, &tile_rect[4]);
        tile_rect[4].x += 640;
    }
    ...
    /*tile_rect[3].y = 330;
    tile_rect[4].y = 250;
    tile_rect[3].x = screen_left % tile_rect[3].w;
    tile_rect[4].x = screen_left % tile_rect[4].w;
    while(tile_rect[3].x <= SCREEN_WIDTH){ //while loop that goes through all the bottom ground tiles and prints the ones in the camera view
        SDL_RenderCopy(obj_renderer, tile_texture[3], NULL, &tile_rect[3]);
        SDL_RenderCopy(obj_renderer, tile_texture[4], NULL, &tile_rect[4]);
        tile_rect[3].x += tile_rect[3].w;
        tile_rect[4].x += tile_rect[4].w;
